...
import com.app.ihe.IPixManagerAdapter;
import com.app.ihe.PixManagerException;
...
class PixQueryHandler extends BaseHandler implements Application {
    ...
    throws ApplicationException, HL7Exception {
        ...
        PatientIdentifier requestPatientId = HL7v25.extractId(composite, connection );
        ...
        boolean requestDomainOk = AssigningAuthorityUtil.validateDomain( requestDomain, connection);
        if (!requestDomainOk) {
        ...
        if (ids.size() >= 1) {
            ...
            for (PatientIdentifier pid : finalIds) {
                 ...
                 Identifier authority = AssigningAuthorityUtil.reconcileIdentifier( 
                		 pid.getAssigningAuthority(), connection );
        ...
        //Audit Log
        auditLog(ids, hl7Header, queryTag, qpd);
     ...
     */
    private void auditLog(List<PatientIdentifier> patientIds, HL7Header hl7Header, String queryTag, QPD qpd) {
    ...
    * @throws ApplicationException if something is wrong with the application
     */
	private RSP_K23 initResponse(HL7Header hl7Header) throws HL7Exception, ApplicationException {
        ...
        HL7v25.populateMSH(reply.getMSH(), "RSP", "K23", getMessageControlId(), serverApplication, serverFacility, idSendingApplication, idSendingFacility );
		return reply;
	}
    ...
    */
	...
	throws HL7Exception, ApplicationException {
        ...
        if (!receivingApplication.equals(getServerApplication()) ) {
        ...
        if (!receivingFacility.equals(getServerFacility()) ) {
            HL7v25.populateMSA(reply.getMSA(), "AE", incomingMessageId);
            HL7v25.populateQAK(reply.getQAK(), queryTag, "AE");

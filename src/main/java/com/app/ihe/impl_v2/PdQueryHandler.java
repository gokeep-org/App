...
import com.app.ihe.HL7Actor;
import com.app.ihe.IPdSupplierAdapter;
import com.app.ihe.PdSupplierException;
import com.app.ihe.audit.ParticipantObject;
import com.app.ihe.configuration.Configuration;
import com.app.ihe.configuration.IheConfigurationException;
import com.app.ihe.data.Patient;
import com.app.ihe.data.PatientIdentifier;
import com.app.ihe.data.PersonName;
import com.app.ihe.impl_v2.hl7.HL7Header;
import com.app.ihe.impl_v2.hl7.HL7Util;
import com.app.ihe.impl_v2.hl7.HL7v25;
import com.app.ihe.log.MessageStore;
import com.app.ihe.pdq.ContinuationPointer;
import com.app.ihe.pdq.PdqQuery;
import com.app.ihe.pdq.PdqResult;
import com.app.ihe.util.AssigningAuthorityUtil;
import com.app.ihe.util.ExceptionUtil;
...
class PdQueryHandler extends BaseHandler implements Application {
    ...
    PdQueryHandler(PdSupplier actor) {
    	...
    	this.actor = actor;
        this.pdqAdapter = actor.getPdqSupplierAdapter();
     ...
     * @throws ApplicationException if anything wrong with the application
     * @throws HL7Exception if anything wrong with the HL7 message format
     */
    private Message processQuery(Message msgIn) throws ApplicationException, HL7Exception {
         ...
         processMSH(hl7Header, reply.getMSH(), "RSP", "K22",  getServerApplication(), getServerFacility());
         ...
         if(isContinuationQueryByOpenPixPdq()){
        	 if (StringUtil.goodString(pointer)) {
                ...
                pdqResult  = getPdqResult(qpd, reply, hl7Header, outTerser, inTerser, pointer,recordRequestNumber,false);
                if (pdqResult == null) 
                	return reply;
         ...
         //Audit Log
         auditLog(finalPatients, hl7Header, queryTag, qpd);
    ...
    * @throws ApplicationException if there is any application error
    */
   private Message processQCN_J01(QCN_J01 qcnMsg) throws DataTypeException, ApplicationException {

	   HL7Header hl7Header = new HL7Header(qcnMsg);

       ACK reply = new ACK();
       MSH replyMsh = reply.getMSH();
       processMSH(hl7Header, replyMsh, "ACK", "J01", getServerApplication(), getServerFacility());
	 ...
	 * @throws ApplicationException if application encounter any problem
	 */
        ...
        for (int patientIndex=0; patientIndex<finalPatients.size(); patientIndex++) {
            ...
            try {
                ...
                for (int i=1; i<patientRecord.size(); i++) { //has to start with the second one, the first was handled above
                    ...
                    List<com.app.ihe.data.PatientIdentifier> patientIds = pd.getPatientIds();
                    for (com.app.ihe.data.PatientIdentifier patientId : patientIds) {
                    	String id = patientId.getId();
                    	Identifier assigningAuthority = patientId.getAssigningAuthority();
                    	assigningAuthority = AssigningAuthorityUtil.reconcileIdentifier( assigningAuthority, connection );
    ...
    */
   private void auditLog(List<List<Patient>> patients, HL7Header hl7Header, String queryTag, QPD qpd) {
    ...
    */
   private void processMSH(HL7Header requestMsh, MSH replyMsh, String messageType, String event, Identifier receivingApplication, Identifier receivingFacility) throws ApplicationException, DataTypeException {
        ...
        HL7v25.populateMSH(replyMsh, messageType, event, getMessageControlId(), receivingApplication, receivingFacility, idSendingApplication, idSendingFacility );
    ...
    * @throws ApplicationException if anything is wrong the application
    */
   ...
   throws HL7Exception, ApplicationException {
	   ...
	   //We need to validate ReceivingApplication and ReceivingFacility.
       ...
       if (!idReceivingApplication.equals(getServerApplication()) ) {
       ...
       if (!idReceivingFacility.equals(getServerFacility()) ) {
           HL7v25.populateMSA(reply.getMSA(), "AE", inMsgControlId);
           HL7v25.populateQAK(reply.getQAK(), queryTag, "AE");
   ...
   private List<Identifier> getReturnDomain(QPD qpd, RSP_K21 reply, String messageControlId, String queryTag, Terser outTerser, Terser inTerser ) throws HL7Exception {
      ...
      for (int i=0; i<typeDomains.length; i++) {
          ...
          if (data instanceof GenericComposite) {
              ...
              boolean validDomain = AssigningAuthorityUtil.validateDomain( aa, connection);
              if (validDomain) {
                  Identifier reconciledDomain = AssigningAuthorityUtil.reconcileIdentifier(aa, connection);
   ...
   private PdqResult getPdqResult(QPD qpd, RSP_K21 reply, HL7Header hl7Header, Terser outTerser, 
		   Terser inTerser ,String pointer,int requestNo, boolean isContinuation) 
       ...
       if (pdqResult == null || pdqResult.getPatients() == null) {
    	   ...
    	   throw new ApplicationException("Failed to get valid PDQ result.");
       }
    ...
    */
    private PdqQuery processQuery(HashMap<String, String> parameters) throws ApplicationException {
    ...
    */
    private int populatePID(PID pid, Patient patient, int setId, List<Identifier> returnDomains) throws HL7Exception, IheConfigurationException, PatientException {
        ...
        for (int i=0; i<pids.size(); i++) {
        	...
        	authority = AssigningAuthorityUtil.reconcileIdentifier( authority, connection );
        ...
        if (null != patient.getMonthersMaidenName()) {
            xpn = pid.getMotherSMaidenName(0);
            PersonName name = patient.getMonthersMaidenName();

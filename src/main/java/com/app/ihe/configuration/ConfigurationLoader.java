 ...
 */
package com.app.ihe.configuration;
...
import com.app.ihe.PatientBroker;
import com.app.ihe.audit.IheAuditTrail;
import com.app.ihe.impl_v2.PdSupplier;
import com.app.ihe.impl_v2.PixManager;
import com.app.ihe.log.IMesaLogger;
import com.app.ihe.log.IMessageStoreLogger;
import com.app.ihe.log.Log4jLogger;
...
public class ConfigurationLoader {
	...
	private boolean createIheActor(String type, String name, IConnectionDescription sourceConnection, 
		...
		} else if (type.equalsIgnoreCase("PixManager")) {
            IConnectionDescription connection = sourceConnection;
            if (connection == null) connection = consumerConnection;
            PixManager pixMan = new PixManager(connection, auditTrail, xdsRegistryConnection, pixConsumerConnections);
        ...
        }  else if (type.equalsIgnoreCase("PdSupplier")) {
            IConnectionDescription connection = sourceConnection;
            if (connection == null) connection = consumerConnection;
            PdSupplier pdSup = new PdSupplier(connection, auditTrail);
            String pdSupplierAdapterClass = Configuration.getPropertyValue(connection, "PdSupplierAdapter", true);
            IPdSupplierAdapter pdsAdapter = null;
            try {
                Class c = Class.forName(pdSupplierAdapterClass);
                pdsAdapter = (IPdSupplierAdapter)getObjectInstance( c );
            } catch (Exception e) {
                String message = "Could not load PdSupplier Adapter in actor type '"+ type +"' in config file " + configFile;
                log.error(message, e);
                throw new IheConfigurationException(message);
            }
            if (pdSup != null) {
                pdSup.registerPdSupplierAdapter( pdsAdapter );
                pdSup.setStoreLogger(getMessageStore(connection, type, configFile));
	...
	public class IheBrokerController implements IBrokerController {
		...
		public boolean shouldUnregister(Object actor) {
            ...
            if (actor instanceof PixManager) return true;
            if (actor instanceof PdSupplier) return true;
